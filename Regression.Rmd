...
When I would read about regression, it would on one hand seem the simplest of approaches to try out and on the other hand, the nuances in each of the steps if well understood and applied, seemed to be waaaay more effortful. Also no one reference tackled even the broad dimensions of applying regression on a dataset. Many aspects would simple be ignored (mostly because they might not be applicable to the dataset), but if you had a new dataset, how would you know if you are missing something or doing something wrong? Hence I wanted a one reference for everything related to Lin Reg. Obviously it took me waaay more than I thought it would, and it is quite long however I still think this would be my refeence for all things linear regression going forward, and hopefully it would help some others too. 
...
1. Statistical significance of the model: 
  ...
  + **t-value** : t-value can be interpreted as the higher the t-value, the lower the chance that the coefficients are non-zero by chance. 
    * Pr(>|t|) indicates the probability that you get t-value as high or higher than the one observed given null hypothesis is true. So if Pr(>|t|) is lower, then the coefficients are significant and vice versa. 
    ...
    where pt = distribution function for the t-distribution with df degrees of freedom and q/x = vector of quantiles [3]
    
2. Model Accuracy:     
  * The actual amount of information in a dataset is indicated by the amount of variance it contains. R2 tells us the amount of variance of the data that is explained by the model. [3]
    + $R^2 = 1 - (SSE/SST)$  
   ...
   * Adjusted $R^2$: As you add variables to the model, the value of $R^2$ will always increase, indepdendent of if the additional variables being significant to the model or not. Hence, we use $R^2$ Adjusted.
